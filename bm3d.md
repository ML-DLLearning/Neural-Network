...
#### 块匹配
根据距离d，设置阈值 $\tau^{\text{ht}}\_{\text{match}}$ ，若 $d<τ$ ，则将这两个块分为一组，得到了某一个组地中心位置坐标的集合：$ S_{x_R}^{ht} = \left\{ x \in X : d(Z_{x_R}, Z_x ) \le \tau\_{match}^{ht} \right\} $ 然后我们对以上分好的组（由2D块堆叠而产生的体）做3D线性变换，并进行硬阈值处理。
...
不清楚这个公式是怎么推导出来的，但是我们可以从先验的角度来理解其道理：令 $t={\left| \mathcal{T}\_{3D}^{\mathrm{wie}}\left( \widehat{\mathbf{Y}}^{\mathrm{basic}}\_{S_{x_R}^{\mathrm{swie}}} \right) \right|^2}$ ，则原式可以化为：
...
\left(
    ...
    \left(
        \mathbf{Z}\_{S_{x\_R}^{\mathrm{swie}}}
    \right)
\right)$
